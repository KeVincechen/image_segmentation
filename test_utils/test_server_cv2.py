...
def get_coordinates(mask):
    contours, _ = cv2.findContours(mask, mode=cv2.RETR_CCOMP, method=cv2.CHAIN_APPROX_TC89_KCOS)
    if not contours:
        return []
    contours = list(map(lambda arr: np.expand_dims(arr.squeeze(1), 0).tolist(), contours))
    to_delete = []
    for i in range(len(_[0])):
        parent_index = _[0][i][-1]
        if parent_index != -1:
            contours[parent_index].extend(contours[i])
            to_delete.append(i)
...
def json_desc_instances(mask, class_names):
    ...
    for j in range(len(class_names)):
        ...
        contours = get_coordinates(mask_single)
